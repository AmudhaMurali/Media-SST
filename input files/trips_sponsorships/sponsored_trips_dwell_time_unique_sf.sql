...
begin;
delete from &{pipeline_schema}.sponsored_trips_dwell_time_unique_sf
where ds between '&{start_dt}' and '&{end_dt}';

insert into &{pipeline_schema}.sponsored_trips_dwell_time_unique_sf
-- Antonio added op1_details 12/13/2023 - (New CTE) we want to retrieve advertiser name, order name and line item name from OP1 given that OP1 has precedence over PIO
with  op1_details as (
...
select
    ...
    dt.marketing_campaign_id

from &{pipeline_schema}.active_sponsored_trips_dwell_time_unique_1 dt
left join (select  trip_id, max(order_id) as order_id from &{pipeline_schema}.trips_orderid_mapping group by 1) map on dt.TRIP_ID = map.TRIP_ID
left join (
