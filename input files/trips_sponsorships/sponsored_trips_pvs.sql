...
begin;
delete from &{pipeline_schema}.sponsored_trips_pvs
where ds between '&{start_dt}' and '&{end_dt}';

insert into &{pipeline_schema}.sponsored_trips_pvs
with    op1_details as (
...
from user_tracking.public.USER_IMPRESSIONS pv

left join (select unique_id, max(LOCALE) as LOCALE, max(os_type) as os_type, max(USER_IP_COUNTRY_NAME) as USER_COUNTRY_NAME,  max(marketing_campaign_id) as marketing_campaign_id
            from USER_TRACKING.PUBLIC.BLESSED_UNIQUE_USERS
          where ds  between '&{start_dt}' and '&{end_dt}'  AND IS_BLESSED = 1
          group by 1 ) uu on uu.unique_id = pv.UNIQUE_ID
-- RIO_SF.rust.a_unique_users u on pv.TA_UNIQUE_ID = u.UNIQUE_ID and pv.ds = u.ds and u.ds between '&{start_dt}' and '&{end_dt}'
JOIN &{pipeline_schema}.active_sponsored_trips_detail trips on cast(trim(CUSTOM_DATA_JSON:tripId, '"') as string) = cast(trips.trip_id as string)
left join  (select trip_id, max(order_id) as order_id from &{pipeline_schema}.trips_orderid_mapping group by 1) map on cast(trim(CUSTOM_DATA_JSON:tripId, '"') as string) = cast(map.TRIP_ID as string)
left join (
...
where  pv.page in('Trips','TripDetails') and ITEM_TYPE = 'TripContent' and ITEM_NAME = 'ViewPublicTrip' and pv.ds >= '2021-08-18' and pv.ds between '&{start_dt}' and '&{end_dt}'
