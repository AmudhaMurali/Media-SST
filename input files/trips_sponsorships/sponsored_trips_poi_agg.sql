...
begin;
delete from &{pipeline_schema}.sponsored_trips_poi_agg
where ds between '&{start_dt}' and '&{end_dt}';

insert into &{pipeline_schema}.sponsored_trips_poi_agg
with    op1_details as (
...
select a.ds,
            ...
            sum(a.poi_map_click)      as poi_map_click,
            sum(a.poi_carousel)       as poi_carousel,
            sum(a.poi_readmore)    as poi_readmore,
            sum(a.poi_clickthr)      as poi_clickthr,
            sum(a.poi_map_clickthr)        as poi_map_clickthr,
            sum(a.poi_save)               as poi_save,
            count(distinct(case when poi_map_click >0 or poi_carousel >0 or poi_readmore >0 or poi_clickthr >0 or poi_map_clickthr >0 or poi_save >0 then a.unique_id else null end) )  as uniques_with_poi_actions
from &{pipeline_schema}.sponsored_trips_poi_actions_new a
