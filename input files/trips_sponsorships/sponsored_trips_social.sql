...
-- filtering cx_analytics.trip_social_actions down to only sponsored trips --

begin;
delete from &{pipeline_schema}.sponsored_trips_social
where ds between '&{start_dt}' and '&{end_dt}';


-----------------------------------------------------------
-- remake of the Woodsy Job trip_social_actions
-- aggregates the likes, reposts and shares for each
-- sponsored trip
-- this is a final table (i.e. to be used in tableau)
-----------------------------------------------------------


insert into &{pipeline_schema}.sponsored_trips_social
with    op1_details as (
...
select s.DS           as ds,
       ...
       s.TITLE        as title,
       s.TRIP_LIKES   as trip_likes,
       s.TRIP_REPOSTS as trip_reposts,
       s.TRIP_SHARES  as trip_shares,
...
from display_ads.public.trip_social_actions s
--left join &{pipeline_schema}.sponsored_trips st on s.trip_id = st.trip_id
JOIN &{pipeline_schema}.active_sponsored_trips_detail trips on  s.trip_id= trips.trip_id
left join  (select  trip_id, max(order_id) as order_id from &{pipeline_schema}.trips_orderid_mapping group by 1) map on s.TRIP_ID = map.TRIP_ID
left join (
...
--where lower(s.username) in (select distinct lower(username) from &{pipeline_schema}.trip_sponsors)
where ds between '&{start_dt}' and '&{end_dt}'
;
