...
begin;
delete from &{pipeline_schema}.sponsored_trips_pageviews
where ds between '&{start_dt}' and '&{end_dt}';

insert into &{pipeline_schema}.sponsored_trips_pageviews
with    op1_details as (
...
select pv.ds,
       COALESCE(try_cast(split_part(regexp_substr(url,'/TripDetails-t\\d*'),'t',3) as int),try_cast(split_part(regexp_substr(url,'/Trips/\\d*'),'/',3) as int)) as trip_id,
    ...
    COALESCE(op.account_exec,pio.account_exec,pio2.account_exec)                                                      as account_exec,
       ...
       count(distinct PAGE_UID) as pageviews

from user_tracking.public.pageviews pv

left join (select unique_id, max(LOCALE) as LOCALE, max(os_type) as os_type, max(USER_IP_COUNTRY_NAME) as USER_COUNTRY_NAME from USER_TRACKING.PUBLIC.BLESSED_UNIQUE_USERS
          where ds  between '&{start_dt}' and '&{end_dt}'  AND IS_BLESSED = 1
          group by 1 ) uu on uu.unique_id = pv.TA_UNIQUE_ID
-- RIO_SF.rust.a_unique_users u on pv.TA_UNIQUE_ID = u.UNIQUE_ID and pv.ds = u.ds and u.ds between '&{start_dt}' and '&{end_dt}'
JOIN &{pipeline_schema}.active_sponsored_trips_detail trips on COALESCE(try_cast(split_part(regexp_substr(url,'/TripDetails-t\\d*'),'t',3) as string),try_cast(split_part(regexp_substr(url,'/Trips/\\d*'),'/',3) as string)) = cast(trips.trip_id as string)
left join  (select trip_id, max(order_id) as order_id from &{pipeline_schema}.trips_orderid_mapping group by 1) map on COALESCE(try_cast(split_part(regexp_substr(url,'/TripDetails-t\\d*'),'t',3) as string),try_cast(split_part(regexp_substr(url,'/Trips/\\d*'),'/',3) as string)) = cast(map.TRIP_ID as string)
left join (
...
where  page_name in ('Trips','TripDetails') and pv.ds >= '2021-08-18' and pv.ds between '&{start_dt}' and '&{end_dt}'
