...
delete from &{pipeline_schema}.dmo_top_location_pv
where ds between '&{start_dt}' and '&{end_dt}';


INSERT INTO &{pipeline_schema}.dmo_top_location_pv
...
select a.month as ds, a.user_country_name_agg, a.user_market, a.top_city_id, a.top_city_name, a.uniques, a.pvs, a.bookings, a.clicks,a.city_rank_pv, total.pvs as total_pvs from
(SELECT * FROM (
        SELECT date_trunc('MONTH',bltg.ds)                                                                                              as month,
               ...
               sum(bltg.bookings)                                                                                                       as bookings,
               sum(bltg.clicks)                                                                                                         as clicks,
               rank() over(partition by month, user_country_name_agg, ctr.region order by sum(bltg.pvs) desc) as city_rank_pv
        FROM DISPLAY_ADS.sales.blt_top_locations_geo bltg
            --JOIN  DISPLAY_ADS.sales.blt_geo_list g on bltg.geo_id = g.geo_id
            JOIN rio_sf.anm.country_to_region ctr on ctr.country = bltg.user_country_name
        WHERE month BETWEEN '&{start_dt}' and '&{end_dt}'  -- manually changed to pull 2020-2022
        GROUP BY 1,2,3,4,5)
WHERE city_rank_pv BETWEEN '1' and '50') a
