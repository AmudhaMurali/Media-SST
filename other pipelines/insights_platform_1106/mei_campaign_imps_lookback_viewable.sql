------------------------------------------------------------------
-- uses impressions (only viewable impressions) as base table
...
delete from &{pipeline_schema}.mei_campaign_imps_lookback_viewable
where ds = '&{start_dt}';

insert into  &{pipeline_schema}.mei_campaign_imps_lookback_viewable
with interactions as (
  ...
  select ds                                 as ds,
         ...
         sum(VIEWABLE_IMPRESSION_COUNTS)    as total_viewable_imps,
         sum(VIEWABLE_CLICK_COUNTS)         as total_viewable_ad_clicks
    from display_ads.sales.user_order_impressions_daily
    where ds = '&{start_dt}'
    and VIEWABLE_IMPRESSION_COUNTS > 0
    group by ds, unique_id, ADVERTISER_ID
)
select imp.ds                                                       as ds
     , imp.UNIQUE_ID is not null                                    as saw_campaign
     , ifnull(sum(imp.total_viewable_imps), 0)                      as imp_total_viewable_ad_impressions
     , ifnull(sum(imp.total_viewable_ad_clicks), 0)                 as imp_total_viewable_ad_clicks
     ...
     , reg.region2_name                                             as SUB_REGION
     --, reg.geo_name                                                 as CITY
...
LEFT JOIN &{pipeline_schema}.dfp_ad_name adname on imp.advertiser_id = adname.advertiser_id
JOIN rio_sf.rust.a_unique_users uu ON imp.unique_id = uu.unique_id AND imp.ds = uu.ds
