------------------------------------------------------------------
-- uses interactions as base table and joins to impressions
-- (only viewable imps) to see if those who have interacted
...
delete from &{pipeline_schema}.mei_campaign_agg_viewable
where ds = '&{start_dt}';

insert into  &{pipeline_schema}.mei_campaign_agg_viewable
WITH impressions as (
...
SELECT inter.ds                                                         as ds
     ...
     , sum(case when inter.num_locations_booked > 0 then 1 else 0 end)  as num_acc_bookers -- user_booked_a_location
...
FROM interactions inter
    LEFT JOIN impressions imp                               on imp.unique_id = inter.unique_id
                                                            ...
                                                            and (imp.last_imp between '&{start_dt_m90}' and '&{start_dt}')
